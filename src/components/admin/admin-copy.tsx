...
export function DashboardCards({
  ...
  cards: { labelKey: 'admin.productsCount'|'admin.pendingOrders'|'admin.customRequests'|'admin.messages'; hintKey: 'admin.manageCatalogue'|'admin.awaitingConfirm'|'admin.newBespoke'|'admin.newContacts'; href: string; value: number; Icon: ComponentType<{ className?: string }> }[]
}) {
  const t = useT()
  return (
      ...
      {cards.map((c) => (
        <Link key={c.labelKey} href={c.href} className="surface-card rounded-xl p-5 transition-colors hover:border-primary/40">
          <div className="flex items-center justify-between">
            <span className="text-primary"><c.Icon className="h-6 w-6" /></span>
